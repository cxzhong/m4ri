...
mzd_t *mzd_from_png_fh(FILE *fh, int verbose) {
  ...
  if (verbose)
    printf("reading %u x %u matrix (bit depth: %u, channels: %u, color type: %u, compression type: "
           "%u)\n",
           (unsigned int)m, (unsigned int)n, (unsigned int)bit_depth, (unsigned int)channels,
           (unsigned int)color_type, (unsigned int)compression_type);
...
int mzd_to_png_fh(const mzd_t *A, FILE *fh, int compression_level, const char *comment,
  ...
  png_set_IHDR(png_ptr, info_ptr, A->ncols, A->nrows, 1, PNG_COLOR_TYPE_GRAY, PNG_INTERLACE_NONE,
...
mzd_t *mzd_from_str(rci_t m, rci_t n, const char *str) {
  ...
  for (rci_t i = 0; i < A->nrows; i++) {
    for (rci_t j = 0; j < A->ncols; j++) { mzd_write_bit(A, i, j, str[idx++] == '1'); }
  }
  return A;
}
